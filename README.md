...
**想把這三個資料夾放在別的地方**，建立設定檔 `~/.config/dropconvert/config.json`：

```json
{"base": "~/Desktop/個人工具/轉檔工具"}
```

`base` 是要放 `待轉檔`、`已轉檔`、`已處理` 的那一層。改完重新打開 App 就生效；
原本資料夾裡的檔案要自己搬過去。設定檔寫錯時會先用「下載項目」，並在視窗上方提示。
設定檔不放在程式資料夾裡，所以更新程式不會把它蓋掉。
